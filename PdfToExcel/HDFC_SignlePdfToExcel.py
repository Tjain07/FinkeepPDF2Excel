...
input_folder = 'C:\\Users\\jaint\\Downloads\\PdfToExcel\\PdfToExcel\\Input\\HDFC'
output_folder = 'C:\\Users\\jaint\\Downloads\\PdfToExcel\\PdfToExcel\\Output\\HDFC'
...
for pdf_filename in os.listdir(input_folder):
    if pdf_filename.endswith('.pdf'):
        ...
        #df = pd.DataFrame(all_data, columns=["Sr No", "Value Date", "Transaction Date", "ChequeNumber", "Transaction Remarks", "DebitAmount", "CreditAmount", "Balance(INR)"])
            df = pd.DataFrame(all_data, columns=["Date", "Narration", "Chq./Ref.No.", "Value Dt", "Withdrawal Amt."])
